    ...
    <ProtectedRoute>
      <div className="p-4 space-y-5 pb-10">
        ...
        <div className="rounded-3xl bg-gradient-to-br from-indigo-500 via-indigo-600 to-purple-700 p-6 text-white shadow-lg shadow-indigo-500/10">
          ...
          <p className="text-3xl font-extrabold mt-1 tracking-tight">{formatCurrency(netWorth, currency)}</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="rounded-2xl bg-[var(--color-surface)] border border-[var(--color-border)] p-4 flex flex-col justify-between shadow-sm">
            ...
            <div className="mt-4">
              <p className="text-lg font-bold text-emerald-500 tracking-tight">{formatCurrency(monthlyIncome, currency)}</p>
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-[var(--color-surface)] border border-[var(--color-border)] p-4 flex flex-col justify-between shadow-sm">
            ...
            <div className="mt-4">
              <p className="text-lg font-bold text-rose-500 tracking-tight">{formatCurrency(monthlyExpense, currency)}</p>
            </div>
          </div>
        </div>
                ...
                return (
                  <div key={categoryId} className="flex items-center justify-between text-xs">
                    ...
                    <span className="text-[var(--color-text-secondary)] font-bold whitespace-nowrap">
                      {formatCurrency(total, currency)} <span className="opacity-75 font-medium ml-1">({percentage}%)</span>
                    </span>
                  </div>
                );
              })}
                ...
                >
                  ...
                  <div className="text-right whitespace-nowrap shrink-0 ml-2">
                    <span className={`text-sm font-extrabold ${
                      ...
                      {item.type === 'expense' ? '-' : item.type === 'transfer' ? '' : '+'}{formatCurrency(item.amount, currency)}
                    </span>
        ...
        {selectedTx && (
          <div className="flex flex-col items-center py-4">
            ...
            <span className={`text-3xl font-extrabold tracking-tight ${
              ...
              {selectedTx.type === 'expense' ? '-' : selectedTx.type === 'transfer' ? '' : '+'}{formatCurrency(selectedTx.amount, currency)}
            </span>
